-- =====================================================================
-- 15_fix_otp_orders_quotation_join.sql
--
-- MUST BE RUN AGAINST THE SAME PRODUCTION DATABASE as 13_/14_.
--
-- Fixes otp_sync_order_from_tracker(): several fields (billing_address,
-- contact_person, shipping_address, amount_with_tax, quotation_number,
-- gst_number) were coming back NULL/incomplete on otp_orders even though
-- the same data is visible in vw_enquiry_master. Root cause: that data
-- doesn't live on lto_enquiries/lto_leads or the tracker row at all — it
-- lives on lto_make_quotations (+ lto_client_master for billing/GST),
-- joined via lto_make_quotations.enquiry_reference_no matching
-- lto_enquiries.enquiry_no (or lto_leads.lead_no for the lead path),
-- case-insensitive/trimmed, picking the latest quotation by created_at.
-- This mirrors exactly how vw_enquiry_master builds its `latest_quotation`
-- CTE. Tracker-level fields (amount_with_tax, quotation_number, etc.) are
-- still preferred when present — the quotation is only used as a fallback.
--
-- Also backfills every existing otp_orders row with the same logic (see
-- bottom of this file) so already-synced orders get fixed retroactively.
-- =====================================================================
...
BEGIN
  ...
  INSERT INTO public.otp_orders (
    ...
    NEW.quotation_value_without_tax, NEW.quotation_value_with_tax,
...
-- ---------------------------------------------------------------------
-- Backfill: recompute the quotation-sourced fields for every otp_orders
-- row that already exists. Deliberately does NOT touch lto_enquiry_tracker
-- / lto_enquiry_tracker_for_leads at all (a no-op UPDATE there would still
-- re-fire the existing Sheet webhook for every historical row) — this
-- updates otp_orders directly, joining back to the quotation tables by
-- source_ref, so nothing on the Lead-To-Order side is touched.
-- ---------------------------------------------------------------------
WITH latest_quotation AS (
  SELECT DISTINCT ON (upper(trim(q.enquiry_reference_no)))
    upper(trim(q.enquiry_reference_no)) AS ref_key,
    q.quotation_no, q.grand_total, q.ship_to_address,
    q.consignee_contact_name, q.consignee_contact_no,
    cm.billing_address, cm.gst_number
  FROM public.lto_make_quotations q
  LEFT JOIN public.lto_client_master cm ON cm.uuid = q.consignee_client_id
...
UPDATE public.otp_orders o
   SET contact_person = COALESCE(o.contact_person, lq.consignee_contact_name),
       phone_number = COALESCE(o.phone_number, lq.consignee_contact_no),
       billing_address = COALESCE(o.billing_address, lq.billing_address),
       shipping_address = COALESCE(o.shipping_address, lq.ship_to_address),
       gst_number = COALESCE(o.gst_number, lq.gst_number),
       amount_with_tax = COALESCE(o.amount_with_tax, lq.grand_total),
       quotation_number = COALESCE(o.quotation_number, lq.quotation_no)
