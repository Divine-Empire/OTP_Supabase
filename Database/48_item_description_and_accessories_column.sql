-- =====================================================================
-- 48_item_description_and_accessories_column.sql
--
-- MUST BE RUN AGAINST THE SAME PRODUCTION DATABASE as 13_/44_/47_.
--
-- 1. otp_orders.items -- each item object now also carries `description`
--    (lto_make_quotation_items.description, the free-text accessories
--    notes typed while making the quotation), so it can be shown in the
--    Item List popups (Order Acceptable, Pro-Forma Invoice, Check
--    Inventory). Merged into the SAME items array rather than a separate
--    column -- description is inherently per-item, so keeping it on the
--    same object avoids having to match two separate jsonb arrays back
--    together by item_name. Only available on the quotation-items path
--    (migration 47's override); the lto_enquiry_items/lto_lead_items
--    fallback path has no description column, so those items simply carry
--    description: null.
--
-- 2. otp_orders.items_accessories -- new, separate jsonb column, [] by
--    default. NOT populated by this trigger -- unlike `items` (sourced
--    from Lead-To-Order), accessories are scanned during the OTP_Supabase
--    Check Inventory stage itself (reusing the existing QR-scan flow,
--    since accessories are Purchase-FMS-Supabase items with their own
--    QR labels already) and written directly by that stage's own API
--    route. This column just needs to exist so it can be read/displayed
--    by every later stage's Pending/History view.
-- =====================================================================

ALTER TABLE public.otp_orders
  ADD COLUMN IF NOT EXISTS items_accessories jsonb NOT NULL DEFAULT '[]'::jsonb;
...
BEGIN
  ...
  IF TG_TABLE_NAME = 'lto_enquiry_tracker' THEN
    ...
    SELECT COALESCE(jsonb_agg(jsonb_build_object('item_name', i.item_name, 'quantity', i.quantity, 'description', NULL)), '[]'::jsonb),
  ...
  -- Now also carries each item's `description` (accessories notes typed
  -- on the quotation) -- see this migration's header.
  IF v_q_quotation_no IS NOT NULL THEN
    ...
    IF v_q_item_count > 0 THEN
      SELECT jsonb_agg(jsonb_build_object('item_name', qi.item_name, 'quantity', qi.quantity, 'description', qi.description)),
