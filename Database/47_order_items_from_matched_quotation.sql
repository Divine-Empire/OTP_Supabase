-- =====================================================================
-- 47_order_items_from_matched_quotation.sql
--
-- MUST BE RUN AGAINST THE SAME PRODUCTION DATABASE as 13_/34_/44_/45_.
--
-- otp_orders.items/total_qty were always computed from
-- lto_enquiry_items/lto_lead_items -- the enquiry/lead's ORIGINAL item
-- list, captured once at enquiry/lead creation time. Confirmed (via code
-- read of Lead-To-Order-Supabase-New/src/pages/Quotation) that revising a
-- quotation there ONLY reads from lto_enquiry_items/lto_lead_items to
-- prefill the form -- it never writes back to them. So if a user adds/
-- changes items while revising a quotation, lto_enquiry_items/lto_lead_items
-- stay stale, and otp_orders.items/total_qty (and the Sheet's "Total Qty")
-- would end up short.
--
-- Fix: after the existing quotation-match logic (34_/44_) resolves which
-- quotation this order actually uses (v_q_quotation_no), prefer that
-- quotation's own items (lto_make_quotation_items, excluding freight/
-- packaging lines) over the enquiry/lead's original item list. Falls back
-- to the enquiry/lead items (unchanged prior behaviour) when no quotation
-- match was found, or the matched quotation has zero real items.
--
-- Freight/packaging lines are excluded by NAME, not lto_make_quotation_
-- items.is_freight -- checked live data: only 5 rows across the whole
-- table have is_freight = true, vs 108 rows whose item_name contains
-- "freight", so the column is essentially unpopulated/unreliable.
-- Lead-To-Order-Supabase-New/src/pages/Quotation/quotation-form.jsx's own
-- logic identifies these by exact name instead (`item.name === "Freight"`,
-- and `name.trim().toUpperCase() === "PACKAGING AND FORWARDING"`) --
...
BEGIN
  ...
  -- original item list -- see migration header. A revision can add/change
  -- items that never make it back into lto_enquiry_items/lto_lead_items.
  IF v_q_quotation_no IS NOT NULL THEN
    ...
    IF v_q_item_count > 0 THEN
      SELECT jsonb_agg(jsonb_build_object('item_name', qi.item_name, 'quantity', qi.quantity)),
