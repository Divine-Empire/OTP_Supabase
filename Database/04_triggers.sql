...
CREATE OR REPLACE FUNCTION trg_set_stage1_planned()
...
DROP TRIGGER IF EXISTS trg_after_order_insert ON otp_orders;
CREATE TRIGGER trg_after_order_insert
  AFTER INSERT ON otp_orders
  FOR EACH ROW EXECUTE FUNCTION trg_set_stage1_planned();


-- 2. TRIGGER: Stage 1 actual_date -> Stage 2 planned_date (Check Inventory)
CREATE OR REPLACE FUNCTION trg_stage1_to_stage2()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE
  v_tat INTEGER;
BEGIN
...
DROP TRIGGER IF EXISTS trg_after_oa_update ON otp_order_acceptable;
CREATE TRIGGER trg_after_oa_update
  BEFORE UPDATE ON otp_order_acceptable
  FOR EACH ROW EXECUTE FUNCTION trg_stage1_to_stage2();
...
CREATE OR REPLACE FUNCTION trg_stage2_to_stage3_or_4()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE
  v_tat INTEGER;
BEGIN
...
DROP TRIGGER IF EXISTS trg_after_ci_update ON otp_check_inventory;
CREATE TRIGGER trg_after_ci_update
  BEFORE UPDATE ON otp_check_inventory
  FOR EACH ROW EXECUTE FUNCTION trg_stage2_to_stage3_or_4();


-- 4. TRIGGER: Stage 3 actual_date -> Stage 4 planned_date (Senior Approval)
CREATE OR REPLACE FUNCTION trg_stage3_to_stage4()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE
  v_tat INTEGER;
BEGIN
...
DROP TRIGGER IF EXISTS trg_after_mr_update ON otp_material_received;
CREATE TRIGGER trg_after_mr_update
  BEFORE UPDATE ON otp_material_received
  FOR EACH ROW EXECUTE FUNCTION trg_stage3_to_stage4();


-- 5. TRIGGER: Dispatch inserted -> Initialize Make Invoice, Calibration (if required), and Update Delivery planned dates
CREATE OR REPLACE FUNCTION trg_dispatch_created_init_stages()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE
  v_tat INTEGER;
...
DROP TRIGGER IF EXISTS trg_after_dispatch_insert ON otp_dispatches;
CREATE TRIGGER trg_after_dispatch_insert
  AFTER INSERT ON otp_dispatches
  FOR EACH ROW EXECUTE FUNCTION trg_dispatch_created_init_stages();


-- 6. TRIGGER: Stage 5 actual_date (Make Invoice) -> Stage 6 planned_date (Warehouse)
CREATE OR REPLACE FUNCTION trg_stage5_to_stage6()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE
  v_tat INTEGER;
BEGIN
...
DROP TRIGGER IF EXISTS trg_after_mi_update ON otp_make_invoice;
CREATE TRIGGER trg_after_mi_update
  BEFORE UPDATE ON otp_make_invoice
  FOR EACH ROW EXECUTE FUNCTION trg_stage5_to_stage6();


-- 7. TRIGGER: Stage 6 actual_date (Warehouse) -> Stage 7 planned_date (Driver Follow-up)
CREATE OR REPLACE FUNCTION trg_stage6_to_stage7()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE
  v_tat INTEGER;
BEGIN
...
DROP TRIGGER IF EXISTS trg_after_wh_update ON otp_warehouse;
CREATE TRIGGER trg_after_wh_update
  BEFORE UPDATE ON otp_warehouse
  FOR EACH ROW EXECUTE FUNCTION trg_stage6_to_stage7();


-- 8. TRIGGER: Stage 9 update (Delivery Note) -> Sync delivered quantity and completion date on master order
CREATE OR REPLACE FUNCTION trg_sync_delivered_quantities()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE
  v_order_id UUID;
...
DROP TRIGGER IF EXISTS trg_after_ud_update ON otp_update_delivery;
CREATE TRIGGER trg_after_ud_update
  BEFORE UPDATE ON otp_update_delivery
  FOR EACH ROW EXECUTE FUNCTION trg_sync_delivered_quantities();


-- 9. TRIGGER: Order Cancel insertion -> Sync total_cancelled_qty on otp_orders
CREATE OR REPLACE FUNCTION trg_sync_cancelled_quantities()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE
  v_total_cancelled NUMERIC;
BEGIN
...
DROP TRIGGER IF EXISTS trg_after_cancel_insert ON otp_order_cancel;
CREATE TRIGGER trg_after_cancel_insert
  AFTER INSERT ON otp_order_cancel
  FOR EACH ROW EXECUTE FUNCTION trg_sync_cancelled_quantities();
...
DROP TRIGGER IF EXISTS trg_orders_updated_at ON otp_orders;
CREATE TRIGGER trg_orders_updated_at
  BEFORE UPDATE ON otp_orders
  FOR EACH ROW EXECUTE FUNCTION trg_set_updated_at();

DROP TRIGGER IF EXISTS trg_users_updated_at ON otp_users;
CREATE TRIGGER trg_users_updated_at
  BEFORE UPDATE ON otp_users
  FOR EACH ROW EXECUTE FUNCTION trg_set_updated_at();

DROP TRIGGER IF EXISTS trg_tat_updated_at ON otp_stage_tat;
CREATE TRIGGER trg_tat_updated_at
