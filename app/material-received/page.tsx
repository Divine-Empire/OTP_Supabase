...
export default function MaterialReceivedPage() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      const response = await fetch("/api/otp-supabase/orders?stage=material_received&status=pending")
  ...
  // Fetch processed orders from Supabase API
  const fetchProcessedOrders = async () => {
    try {
      const response = await fetch("/api/otp-supabase/orders?stage=material_received&status=history")
      const result = await response.json()
  ...
  const filterOrdersByUserRole = (orders: any[], currentUser: any) => {
    ...
    if (currentUser.assignedSteps?.includes("all") || currentUser.assignedSteps?.includes("material-received")) {
      return orders;
    }
...
const filteredProcessedOrders = useMemo(() => {
    ...
    try {
      ...
      const updateResponse = await fetch("/api/otp-supabase/orders", {
        ...
        body: JSON.stringify({
          ...
          stageData: {
            received_date: inventoryData.receivedDate || null,
  ...
  const handleSubmit = async () => {
    ...
    if (success) {
      ...
      alert(`Order ${selectedOrder.id || selectedOrder.orderNo} has been updated successfully.`)
    }
  }
  ...
  const updateUnavailableItem = (index: number, field: string, value: any) => {
